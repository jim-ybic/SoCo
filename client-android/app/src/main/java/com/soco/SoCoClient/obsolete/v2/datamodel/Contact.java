...
import com.soco.SoCoClient.obsolete.v2.businesslogic.http.task.CreateContactOnServerJob;
...
public class Contact {
    ...
    public Contact(Context context){
        ...
        this.contactIdLocal = DataConfig2.ENTITIY_ID_NOT_READY;
        this.contactIdServer = DataConfig2.ENTITIY_ID_NOT_READY;
        this.contactServerStatus = DataConfig2.CONTACT_SERVER_STATUS_UNKNOWN;
    ...
    public Contact(Cursor cursor){
        ...
        this.contactIdLocal = cursor.getInt(cursor.getColumnIndex(DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL));
        this.contactIdServer = cursor.getInt(cursor.getColumnIndex(DataConfig2.COLUMN_CONTACT_CONTACTIDSERVER));
        this.contactEmail = cursor.getString(cursor.getColumnIndex(DataConfig2.COLUMN_CONTACT_CONTACTEMAIL));
        this.contactUsername = cursor.getString(cursor.getColumnIndex(DataConfig2.COLUMN_CONTACT_CONTACTUSERNAME));
        this.contactServerStatus = cursor.getString(cursor.getColumnIndex(DataConfig2.COLUMN_CONTACT_CONTACTSERVERSTATUS));
        Log.v(tag, "created contact from cursor: " + toString());
    }

    public void save(){
        if(contactIdLocal == DataConfig2.ENTITIY_ID_NOT_READY){
    ...
    void saveNew(){
        ...
        int cidLocal = -1;
        String query = "select max (" + DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL
                + ") from " + DataConfig2.TABLE_CONTACT;
    ...
    void update(){
        ...
        try {
            db.beginTransaction();
            ContentValues cv = new ContentValues();
            cv.put(DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL, contactIdLocal);
            cv.put(DataConfig2.COLUMN_CONTACT_CONTACTIDSERVER, contactIdServer);
            cv.put(DataConfig2.COLUMN_CONTACT_CONTACTEMAIL, contactEmail);
            cv.put(DataConfig2.COLUMN_CONTACT_CONTACTUSERNAME, contactUsername);
            cv.put(DataConfig2.COLUMN_CONTACT_CONTACTSERVERSTATUS, contactServerStatus);
            db.update(DataConfig2.TABLE_CONTACT, cv,
                    DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL + " = ?",
    ...
    public void refresh(){
        ...
        String query = "select * from " + DataConfig2.TABLE_CONTACT
                + " where " + DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL + " = ?";
    ...
    public void delete(){
        ...
        if(contactIdLocal == DataConfig2.ENTITIY_ID_NOT_READY){
            ...
            db.delete(DataConfig2.TABLE_CONTACT,
                    DataConfig2.COLUMN_CONTACT_CONTACTIDLOCAL + " = ?",
                    new String[]{String.valueOf(contactIdLocal)});
