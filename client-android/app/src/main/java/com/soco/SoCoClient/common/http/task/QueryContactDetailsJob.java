...
public class QueryContactDetailsJob extends AsyncTask<Void, Void, Boolean>{
    ...
    String getUrl(){
        ...
        String path = com.soco.SoCoClient.common.http.Config.SERVER_PATH_QUERY_CONTACT_DETAIL;
    ...
    boolean parse(Object response){
        ...
        try {
            ...
            if(isSuccess.equals(com.soco.SoCoClient.common.http.Config.JSON_VALUE_SUCCESS)){
                ...
                String user = data.getString(com.soco.SoCoClient.common.http.Config.JSON_KEY_USER);
                if(user != null && !user.isEmpty()){
                    JSONObject userData = new JSONObject(user);
                    int contactIdServer = userData.getInt(com.soco.SoCoClient.common.http.Config.JSON_KEY_ID);
                    String contactUsername = userData.getString(com.soco.SoCoClient.common.http.Config.JSON_KEY_NAME);
                ...
                contact.setContactServerStatus(com.soco.SoCoClient.common.database.Config.CONTACT_SERVER_STATUS_INVALID);
                contact.save();
                Log.d(tag, "updated contact details with server response: " + contact.toString());
                return false;
            }
