...
public class InviteContactJoinTaskJob extends AsyncTask<Void, Void, Boolean>{
    ...
    String getUrl(){
        ...
        String path = com.soco.SoCoClient.common.http.Config.SERVER_PATH_INVITE_CONTACT_JOIN_TASK;
    ...
    boolean parse(Object response){
        ...
        try {
            ...
            if(isSuccess.equals(com.soco.SoCoClient.common.http.Config.JSON_VALUE_SUCCESS)){
                task.setMemberStatus(contact, com.soco.SoCoClient.common.database.Config.PARTY_JOIN_ACTIVITY_STATUS_INVITED);
                task.save();
                Log.d(tag, "server response success, update member [" + contact.toString() + "] status: " + com.soco.SoCoClient.common.database.Config.PARTY_JOIN_ACTIVITY_STATUS_INVITED);
            }else {
