...
public class AppNetServerHandler extends SimpleChannelInboundHandler<String> {
    ...
    public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
        ctx.pipeline().get(SslHandler.class).handshakeFuture().addListener(new GenericFutureListener<Future<Channel>>() {
                    @Override
                    public void operationComplete(Future<Channel> future) throws Exception {
                        Log.infor("inactive of "+ctx.channel().remoteAddress());
    ...
    protected void channelRead0(ChannelHandlerContext ctx, String msg) throws Exception {
        ...
        for (Channel c: channels) {
            try{
                try{
                    //str = showMsgData(msg);
                    //asm.recvMsgHandler(str);
                	System.out.println("Server received: " + msg);
                }catch(Exception e){
                    Log.infor(e.getMessage());
                }
                if (c != ctx.channel()) {
